...
- **Always announce the build number + codename after a merge or
  push.** After merging a PR or pushing to main, the assistant
  must tell the user which **build number** is now deploying —
  this is the GitHub PR number that just landed — AND the
  **codename** for that build, generated by
  `assets/core/build-codename.js`'s `buildCodename(run)`. Format:
  `"PR #97 merged → build #97 · ruby-flagellum is deploying"`.
  The codename is also embedded in the in-app build stamp so the
  user can confirm the deployed version on refresh. Do this on
